...
#[tauri::command]
pub async fn delete_saved_query(state: State<'_, AppState>, id: i64) -> Result<(), AppError> {
    let local_db = state.local_db.lock().await;
    if let Some(db) = local_db.as_ref() {
...
#[tauri::command]
pub async fn get_saved_queries(state: State<'_, AppState>) -> Result<Vec<SavedQuery>, AppError> {
    let local_db = state.local_db.lock().await;
    if let Some(db) = local_db.as_ref() {
