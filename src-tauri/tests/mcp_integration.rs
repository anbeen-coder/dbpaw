...
fn test_mcp_sql_safety_drop_blocked() {
    ...
    let response = send_request(
        &mut proc,
        r#"{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"dbpaw_execute_query","arguments":{"connection_id":1,"sql":"DROP TABLE users"}}}"#,
    );
    let v: Value = serde_json::from_str(&response).unwrap();
...
fn test_mcp_sql_safety_insert_blocked() {
    ...
    let response = send_request(
        &mut proc,
        r#"{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"dbpaw_execute_query","arguments":{"connection_id":1,"sql":"INSERT INTO users VALUES (1)"}}}"#,
    );
    let v: Value = serde_json::from_str(&response).unwrap();
...
fn test_mcp_sql_safety_multiple_statements_blocked() {
    ...
    let response = send_request(
        &mut proc,
        r#"{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"dbpaw_execute_query","arguments":{"connection_id":1,"sql":"SELECT 1; DROP TABLE users"}}}"#,
    );
    let v: Value = serde_json::from_str(&response).unwrap();
...
fn test_mcp_initialized_notification() {
    ...
    assert!(
        v.get("error").is_none(),
        "server should still be alive after notification"
    );
    assert_eq!(v["result"], serde_json::json!({}));
...
fn test_mcp_method_not_found() {
    ...
    let response = send_request(
        &mut proc,
        r#"{"jsonrpc":"2.0","id":2,"method":"unknown_method","params":{}}"#,
    );
    let v: Value = serde_json::from_str(&response).unwrap();
...
fn test_mcp_resources_list() {
    ...
    let names: Vec<&str> = resources
        ...
        .collect();
    assert!(
        names.contains(&"connections"),
        "Should contain 'connections' resource"
    );
...
fn test_mcp_resources_templates_list() {
    ...
    assert!(
        templates.len() >= 2,
        "Expected at least 2 resource templates"
    );
    ...
    let names: Vec<&str> = templates
        ...
        .collect();
    assert!(
        names.contains(&"table_list"),
        "Should contain 'table_list' template"
    );
    assert!(
        names.contains(&"table_detail"),
        "Should contain 'table_detail' template"
    );
...
fn test_mcp_resources_read_connections() {
    ...
    let response = send_request(
        &mut proc,
        r#"{"jsonrpc":"2.0","id":2,"method":"resources/read","params":{"uri":"dbpaw://connections"}}"#,
    );
    let v: Value = serde_json::from_str(&response).unwrap();
...
fn test_mcp_resources_read_invalid_uri() {
    ...
    let response = send_request(
        &mut proc,
        r#"{"jsonrpc":"2.0","id":2,"method":"resources/read","params":{"uri":"invalid://unknown"}}"#,
    );
    let v: Value = serde_json::from_str(&response).unwrap();
...
fn test_mcp_prompts_list() {
    ...
    let prompt_names: Vec<&str> = prompts
        ...
        .collect();
    assert!(
        prompt_names.contains(&"analyze_table"),
        "Should contain 'analyze_table' prompt"
    );
...
fn test_mcp_prompts_get_unknown() {
    ...
    let response = send_request(
        &mut proc,
        r#"{"jsonrpc":"2.0","id":2,"method":"prompts/get","params":{"name":"nonexistent_prompt","arguments":{}}}"#,
    );
    let v: Value = serde_json::from_str(&response).unwrap();
...
fn test_mcp_completion_complete() {
    ...
    let response = send_request(
        &mut proc,
        r#"{"jsonrpc":"2.0","id":2,"method":"completion/complete","params":{"ref":{"type":"ref/prompt","name":"analyze_table"},"argument":{"name":"connection_id","value":""}}}"#,
    );
    let v: Value = serde_json::from_str(&response).unwrap();
    ...
    assert_eq!(v["id"], 2);
    assert!(
        v["result"]["completion"]["values"].is_array()
            || v.get("error").is_some()
...
fn test_mcp_completion_unknown_arg() {
    ...
    let response = send_request(
        &mut proc,
        r#"{"jsonrpc":"2.0","id":2,"method":"completion/complete","params":{"ref":{"type":"ref/prompt","name":"analyze_table"},"argument":{"name":"nonexistent_arg","value":"foo"}}}"#,
    );
    let v: Value = serde_json::from_str(&response).unwrap();
    ...
    assert!(
        values.is_empty(),
        "Values should be empty for unknown argument"
    );
...
fn test_mcp_sampling_create_message() {
    ...
    let response = send_request(
        &mut proc,
        r#"{"jsonrpc":"2.0","id":2,"method":"sampling/createMessage","params":{"messages":[{"role":"user","content":{"type":"text","text":"hello"}}]}}"#,
    );
    let v: Value = serde_json::from_str(&response).unwrap();

    assert!(
        v.get("error").is_some() || v["result"]["isError"] == true,
        "Should return error for unsupported sampling"
    );
...
fn test_mcp_sql_safety_truncate_blocked() {
    ...
    let response = send_request(
        &mut proc,
        r#"{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"dbpaw_execute_query","arguments":{"connection_id":1,"sql":"TRUNCATE TABLE users"}}}"#,
    );
    let v: Value = serde_json::from_str(&response).unwrap();
...
fn test_mcp_sql_safety_alter_blocked() {
    ...
    let response = send_request(
        &mut proc,
        r#"{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"dbpaw_execute_query","arguments":{"connection_id":1,"sql":"ALTER TABLE users ADD COLUMN age INT"}}}"#,
    );
    let v: Value = serde_json::from_str(&response).unwrap();
...
fn test_mcp_sql_safety_create_blocked() {
    ...
    let response = send_request(
        &mut proc,
        r#"{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"dbpaw_execute_query","arguments":{"connection_id":1,"sql":"CREATE TABLE test (id INT)"}}}"#,
    );
    let v: Value = serde_json::from_str(&response).unwrap();
...
fn test_mcp_sql_safety_update_no_where_blocked() {
    ...
    let response = send_request(
        &mut proc,
        r#"{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"dbpaw_execute_query","arguments":{"connection_id":1,"sql":"UPDATE users SET name = 'test'"}}}"#,
    );
    let v: Value = serde_json::from_str(&response).unwrap();
    ...
    assert!(
        ...
        "UPDATE without WHERE should be blocked as write operation"
    );
...
fn test_mcp_sql_safety_delete_no_where_blocked() {
    ...
    let response = send_request(
        &mut proc,
        r#"{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"dbpaw_execute_query","arguments":{"connection_id":1,"sql":"DELETE FROM users"}}}"#,
    );
    let v: Value = serde_json::from_str(&response).unwrap();
    ...
    assert!(
        text.contains("Write operation"),
        "DELETE without WHERE should be blocked as write operation"
    );
...
fn test_mcp_sql_safety_select_where_allowed() {
    ...
    let response = send_request(
        &mut proc,
        r#"{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"dbpaw_execute_query","arguments":{"connection_id":1,"sql":"SELECT * FROM users WHERE id = 1"}}}"#,
    );
    let v: Value = serde_json::from_str(&response).unwrap();

    let text = v["result"]["content"][0]["text"].as_str().unwrap();
    assert!(
        !text.contains("Dangerous keyword"),
        "SELECT with WHERE should not be blocked by safety checks"
    );
    assert!(
        !text.contains("Write operation"),
        "SELECT with WHERE should not be blocked by safety checks"
    ...
    assert!(
        !text.contains("Multiple statements"),
        "SELECT with WHERE should not be blocked by safety checks"
    );
...
fn test_mcp_tools_call_missing_name() {
    ...
    assert!(
        v.get("error").is_some() || v["result"]["isError"] == true,
        "Should return error when name missing"
    );
