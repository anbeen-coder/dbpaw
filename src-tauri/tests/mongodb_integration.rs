...
use dbpaw_lib::db::drivers::mongodb::MongoDBDriver;
...
async fn test_mongodb_connection_and_list_databases() {
    ...
    let client = MongoDBDriver::connect(&form).await.expect("connect client");
    ...
    let databases = client.list_databases_info().await.expect("list_databases");
    assert!(
        !databases.is_empty(),
